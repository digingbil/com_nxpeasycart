...
- **[TODO for next session]** Checkout UI still shows the generic "Unable to complete the request right now." even when the backend returns specific messages (e.g., out-of-stock). Frontend bundle needs rebuilding and/or additional wiring to surface `data.message` from the payment response. Investigate `media/com_nxpeasycart/src/site/islands/checkout.js` and ensure the built JS is deployed.
