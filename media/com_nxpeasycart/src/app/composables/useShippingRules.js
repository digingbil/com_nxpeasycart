...
export function useShippingRules({ endpoints, token, preload = {} }) {
  ...
  const saveRule = async (payload) => {
    ...
    try {
      const rule = payload.id
        ? await api.updateShippingRule({ endpoint: updateEndpoint, id: payload.id, data: payload })
        : await api.createShippingRule({ endpoint: createEndpoint, data: payload });

      if (rule) {
        const index = state.items.findIndex((item) => item.id === rule.id);
  ...
  const deleteRules = async (ids) => {
    ...
    try {
      const deleted = await api.deleteShippingRules({ endpoint: deleteEndpoint, ids });
