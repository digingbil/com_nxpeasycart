...
export function usePayments({ endpoints = {}, token = '' }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      state.config = normalize(payload?.data?.config ?? payload?.config ?? {});
    } catch (error) {
      state.error = error.message ?? 'Unable to load payment configuration.';
    } finally {
      state.loading = false;
    }
  };
  ...
  const save = async (config) => {
    ...
    try {
      ...
      state.error = error.message ?? 'Unable to save payment configuration.';
    } finally {
      state.saving = false;
    }
  };
