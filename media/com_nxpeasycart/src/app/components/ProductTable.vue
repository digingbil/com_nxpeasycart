  ...
  <table class="nxp-admin-table" aria-describedby="nxp-products-caption">
    ...
    <thead>
      <tr>
        ...
        <th scope="col">{{ __('COM_NXPEASYCART_PRODUCTS_TABLE_TITLE', 'Product') }}</th>
        <th scope="col">{{ __('COM_NXPEASYCART_PRODUCTS_TABLE_PRICE', 'Price') }}</th>
        <th scope="col">{{ __('COM_NXPEASYCART_PRODUCTS_TABLE_VARIANTS', 'Variants') }}</th>
        <th scope="col">{{ __('COM_NXPEASYCART_PRODUCTS_TABLE_CATEGORIES', 'Categories') }}</th>
        <th scope="col">{{ __('JSTATUS', 'Status') }}</th>
        <th scope="col">{{ __('COM_NXPEASYCART_PRODUCTS_TABLE_UPDATED', 'Updated') }}</th>
    ...
    <tbody>
      ...
      <tr v-for="item in items" :key="item.id">
        <td>{{ item.id }}</td>
        <td>
          <div class="nxp-products-table__title">{{ item.title }}</div>
        ...
        <td>
          ...
          <span v-else>
            {{ __('COM_NXPEASYCART_PRODUCTS_NO_CATEGORIES', 'Uncategorised') }}
          </span>
        </td>
        <td>
...
const variantPrice = (product) => {
  ...
  const minCents = Number.isFinite(summary.price_min_cents) ? summary.price_min_cents : null;
  const maxCents = Number.isFinite(summary.price_max_cents) ? summary.price_max_cents : null;

  if (minCents === null) {
    return '';
  }

  const currency = summary.currency || product?.variants?.[0]?.currency || baseCurrency.value;
...
const variantCountLabel = (product) => {
  const summary = product?.summary?.variants ?? {};
  const count = Number.isFinite(summary.count) ? summary.count : (Array.isArray(product?.variants) ? product.variants.length : 0);

  if (count === 1) {
    return __('COM_NXPEASYCART_PRODUCTS_VARIANT_COUNT_ONE', '1 variant');
  }

  return __('COM_NXPEASYCART_PRODUCTS_VARIANT_COUNT', '%s variants', [String(count)]);
};
